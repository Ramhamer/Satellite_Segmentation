...
def train(cfg,rank=None, world_size=None): #Pull all the vars from the config file
    #cfg
...
####################################################################################
    ...
    #class description 
    # if criterion_name not in ["CrossEntropyLoss","DiceCrossEntropyLoss"]:
...
if __name__ == "__main__":
    ...
    if world_size > 1:
        mp.spawn(train(cfg), args=(world_size,), nprocs=world_size, join=True)
    else:
        rank = 0
